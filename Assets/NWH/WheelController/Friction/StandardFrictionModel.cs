...
namespace NWH.WheelController3D
{
    ...
    public class StandardFrictionModel : IFrictionModel
    {
            ...
            AnimationCurve frictionCurve, float BCDEz, float kFx, float kSx, ref float Sx, ref float Fx, ref float Tcnt)
        {
            ...
            Tb = Tb * (W > 0 ? -1f : 1f);
            float brakeTorqueCap = (W < 0 ? -W : W) * I / dt;
            Tb = Tb > brakeTorqueCap  ? brakeTorqueCap :
                 Tb < -brakeTorqueCap ? -brakeTorqueCap : Tb;
            W += Tb / I * dt;
            ...
            Fx =  surfaceTorque / R * kFx;

            float deltaOmegaTorque = (W - Winit) * I / dt;
            Tcnt = -surfaceTorque + Tb - deltaOmegaTorque;
