...
namespace NWH.WheelController3D
{
    ...
    public class Wheel
    {
        ...
        public void Initialize(WheelController wc)
        {
            ...
            {
                Debug.LogWarning($"An error has happened while setting up RimCollider. RimCollider might have not been generated. Error: {e}");
            }
        }
        ...
        private Mesh GenerateRimColliderMesh(Transform rt)
        {
            ...
            for (theta = startAngleOffset; theta <= Mathf.PI * 2 + startAngleOffset; theta += Mathf.PI / 12.0f)
            {
                ...
                {
                    x = radius * 1.08f * Mathf.Cos(theta);
                    y = radius * 1.08f * Mathf.Sin(theta);
                }
                else
                {
                    x = radius * 0.06f * Mathf.Cos(theta);
                    y = radius * 0.06f * Mathf.Sin(theta);
                }
